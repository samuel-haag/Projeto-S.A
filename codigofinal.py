...
from tkinter import ttk, messagebox  # Importa ttk (para widgets avançados) e messagebox (para caixas de diálogo)
import json  # Importa o módulo json para manipular dados no formato JSON

# Função para realizar o login
def realizar_login():
    usuario = entry_usuario.get()  # Obtém o texto inserido no campo de entrada de usuário
    senha = entry_senha.get()  # Obtém o texto inserido no campo de entrada de senha

    try:
        with open("usuarios.json", "r") as f:  # Tenta abrir o arquivo de usuários
            usuarios = json.load(f)  # Carrega os dados JSON do arquivo
    except FileNotFoundError:  # Caso o arquivo não exista
        usuarios = {}  # Cria um dicionário vazio do (usuário)

    if usuario in usuarios and usuarios[usuario] == senha:  # Verifica se o usuário e senha coincidem
        messagebox.showinfo("Login", "Login realizado com sucesso!")  # Mostra mensagem de sucesso
        abrir_tela_estoque()  # Abre a tela de gerenciamento de estoque
    else:
        messagebox.showerror("Erro", "Usuário ou senha incorretos.")  # Mostra mensagem de erro

# Função para realizar cadastro de um novo usuário
def realizar_cadastro():
    usuario = entry_usuario.get()  # Obtém o texto inserido no campo de entrada de usuário
    senha = entry_senha.get()  # Obtém o texto inserido no campo de entrada de senha

    if usuario and senha:  # Verifica se os campos foram preenchidos
        try:
            with open("usuarios.json", "r") as f:  # Tenta abrir o arquivo de usuários
                usuarios = json.load(f)  # Carrega os dados JSON do arquivo
        except FileNotFoundError:  # Caso o arquivo não exista
            usuarios = {}  # Cria um dicionário vazio

        if usuario in usuarios:  # Verifica se o usuário já existe
            messagebox.showerror("Erro", "Usuário já existe.")  # Mostra mensagem de erro
            return

        usuarios[usuario] = senha  # Adiciona o novo usuário e senha ao dicionário.
        with open("usuarios.json", "w") as f:  # Abre o arquivo para escrita.
            json.dump(usuarios, f)  # Salva os dados no arquivo.
        messagebox.showinfo("Cadastro", f"Usuário '{usuario}' cadastrado com sucesso!")  # Mostra mensagem de sucesso.
    else:
        messagebox.showerror("Erro", "Preencha todos os campos para se cadastrar.")  # Mostra mensagem de erro.

# Função para abrir a tela de gerenciamento de estoque.
def abrir_tela_estoque():
    janela.destroy()  # Fecha a janela de login/cadastro.
    clientes = []  # Lista para armazenar os clientes.

    estoque = tk.Tk()  # Cria a janela principal para o gerenciamento de estoque.
